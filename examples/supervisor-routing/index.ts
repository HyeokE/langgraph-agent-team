...
interface SupervisorState {
  stage: "triage" | "research" | "respond" | "done";
  task: string;
...
const supervisor: TeamAgent<SupervisorState, SupervisorState, string> = {
  id: "supervisor",
...
const researcher: TeamAgent<SupervisorState, SupervisorState, string> = {
  id: "researcher",
  run: async (ctx) => ({
    state: {
      ...ctx.state,
...
const responder: TeamAgent<SupervisorState, SupervisorState, string> = {
  id: "responder",
  run: async (ctx) => {
...
async function main(): Promise<void> {
  const factory = createTeamFactory<SupervisorState, SupervisorState, string>({
    stateSchema,
    hooks: {
      onRoute: (event) => {
