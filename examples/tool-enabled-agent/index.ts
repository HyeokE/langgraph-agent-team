...
const tools: Record<string, (query: string) => string> = {
  lookup: (query) => `Tool lookup result for: ${query}`
};

const supervisor: TeamAgent<ToolState, ToolState, string> = {
  id: "supervisor",
  run: async (ctx) => {
...
const toolAgent: TeamAgent<ToolState, ToolState, string> = {
  id: "tool-agent",
  run: async (ctx) => {
...
const answerAgent: TeamAgent<ToolState, ToolState, string> = {
  id: "answer-agent",
  run: async (ctx) => {
...
async function main(): Promise<void> {
  const factory = createTeamFactory<ToolState, ToolState, string>({
    stateSchema,
    modelAdapter: {
